...
def habituation_run(df):
    ...
    while mode == 'h':
        ...
        while "Status: Ready" not in line:
    ...
    if mode == 'h':
        ...
        for r in xrange(repeats):
            ...
            while trial_num < len(trials):
                trialType = trials[trial_num]

                #THE HANDSHAKE
                # send all current parameters to the arduino box to run the trial
                params = {
                    'trialType'         : trialType,
                    #the value of 'frequency' is actually the frequency
                    'frequency'         : 20 if trialType == 'N' else 200,
                }
                ...
                if not pd.isnull(df['frequency'].iloc[-1]):
